/**
 * AGE YT# Content Strategy — app.js
 * Vanilla JS, tanpa framework, supaya ringan di HP/laptop spek rendah.
 * Adaptasi engine dari AGE YT# Script & Caption Studio (Tools #1).
 */
...
// ─── Nav sidebar (channel strip ala mixer) ────────────────────────────────────
...
function renderForm() {
  ...
  // extra field (select dropdown, text input, atau textarea)
  let extraHtml = "";
  if (f.extraField) {
    const ef = f.extraField;
    if (ef.options) {
      extraHtml = `<label class="field-label" for="extra-field">${ef.label}</label>
         <select id="extra-field" class="field-input">
           ${ef.options.map(o => `<option value="${o}">${o}</option>`).join("")}
         </select>`;
    } else if (ef.type === "textarea") {
      extraHtml = `<label class="field-label" for="extra-field">${ef.label}</label>
         <textarea id="extra-field" class="field-input" rows="3" placeholder="${ef.placeholder || ""}"></textarea>`;
    } else {
      extraHtml = `<label class="field-label" for="extra-field">${ef.label}</label>
  ...
  root.innerHTML = `
    <label class="field-label" for="topic-input">${f.topicLabel || "Topik / ide konten"} <span class="req">*</span></label>
    <textarea id="topic-input" class="field-input" rows="3"
      placeholder="${f.topicPlaceholder || "Contoh: tips hemat belanja bulanan untuk anak kos"}"></textarea>
  ...
  // audience chip click (scoped ke #aud-chips saja — yt-format-chips pakai class sama)
...
async function handleGenerate(isRetry) {
  ...
  if (!topic) {
    showError("Kolom pertama tidak boleh kosong. Isi dulu ya sebelum generate.");
    return;
  }
  ...
  const audLabel = (AUDIENCE_OPTIONS.find(a => a.value === audience) || {}).label || audience;
  const ctx = { topic, platform: platformLabel, detail, audience: audLabel };
...
function showOutput(feature, text, audience, finishReason) {
  ...
  meta.innerHTML = `<span class="aud-badge">${audience}</span>`;
...
// Kirim ke AGE YT#1 (Script & Caption Studio) atau AGE YT#5 (MPT) — format JSON standar antar-tool AGE YT
function sendToMPT() {
  if (!lastOutput.text) return;
  const payload = {
    source       : "AGE YT# Content Strategy",
  ...
  navigator.clipboard.writeText(JSON.stringify(payload, null, 2))
    .then(() => flash("send-mpt-btn", "✅ Siap ditempel di tools AGE YT# lain!"));
}
